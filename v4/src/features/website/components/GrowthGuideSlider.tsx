        ...
        {guides.map((g) => {
          ...
          return (
            <button key={g.id} onClick={() => setSelected(g)}
              className="w-full text-left rounded-2xl p-5 h-full border border-gray-100 hover:shadow-md transition-all active:scale-[0.98] flex flex-col justify-between"
              ...
              <div>
                {g.subtitle && <p className="text-sm text-gray-500 line-clamp-2 leading-relaxed mb-3">{g.subtitle}</p>}
                <span className="inline-block text-xs font-semibold text-[#0F6E56]">자세히 보기 →</span>
              </div>
            </button>
          );
        })}
