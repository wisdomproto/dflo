...
const BEST_POST_TIMES: Record<string, Partial<Record<PublishChannel, string>>> = {
  ko: { instagram: '평일 19~21시 / 일 11시', facebook: '평일 12~13시', threads: '평일 20~22시' },
  th: { instagram: '19~21시 (ICT)', facebook: '12~13시 (ICT)' },
};
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-3">
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        <span className={`ml-auto rounded-full px-2 py-0.5 text-xs ${STATUS_COLORS[it.status]}`}>
      ...
      {it.status === 'published' && (
        <div className="mt-2 flex flex-wrap items-center gap-3 text-xs">
          ...
          {it.publishedUrl && (
            <a href={it.publishedUrl} target="_blank" rel="noreferrer" className="text-[#4A2D6B] underline hover:opacity-80">
              보기 ↗
            </a>
          )}
