...
// rows 전달 시 textarea(나레이션 등 여러 줄) — Enter 는 줄바꿈이라 blur 로만 커밋.
...
export function CommitInput({
  value, onCommit, placeholder, className, rows,
}: { value: string; onCommit: (v: string) => void; placeholder?: string; className?: string; rows?: number }) {
  ...
  if (rows) {
    return (
      <textarea
        ...
        onBlur={commit}
        className={className ?? inputCls + ' resize-none leading-relaxed'}
      />
    );
  }
  return (
