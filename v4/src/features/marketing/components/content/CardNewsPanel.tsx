...
function slidePromptText(s: CardnewsSlide, lang: CardLang): string {
  ...
  if (t.subtext) p += `\n[보조문구] 「${t.subtext}」`;
  if (s.isCta) {
    p += '\n[로고 처리] 첨부한 로고를 그대로 사용한다. 로고의 글자·색·형태·비율을 절대 바꾸지 말고(다시 그리거나 재배색·재디자인 금지), 원본 PNG 그대로 배치만 할 것.';
    p += '\n  - 로고는 배경 투명 PNG다. 상단에 아래→위로 흰색이 차오르는 부드러운 세로 그라데이션 밴드를 깔고(맨 위 불투명한 흰색, 아래로 갈수록 투명해져 보라 배경과 자연스럽게 섞임), 그 흰 영역 위에 로고를 원본 그대로 중앙 배치.';
    p += '\n  - 딱딱한 사각 테두리나 붙여넣은 듯한 흰 박스, 그림자·외곽선 금지. 흰 밴드 덕분에 로고가 배경과 자연스럽게 어우러지게 하되, 로고 자체에는 손대지 않는다.';
    p += `\n[도메인·하단] 「${DOMAINS[lang]}」`;
  }
  return p;
}
