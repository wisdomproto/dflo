...
function buildHtml(a: BlogSeoArticle, lang: string): string {
...
export function BlogPreviewModal({ article, language, onClose }: { article: BlogSeoArticle; language: string; onClose: () => void }) {
  const [device, setDevice] = useState<'mobile' | 'pc'>('mobile');
  const html = useMemo(() => buildHtml(article, language), [article, language]);
  // 배경 스크롤 잠금
  useEffect(() => {
