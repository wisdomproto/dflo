...
window.addEventListener('message', function (e) {
  ...
  var i18n = window.__I18N__ || {};
  var allowed = ['ko', 'th', 'vi', 'en', 'zh-hant', 'zh-hans'];
...
// ============= 1:1 상담 페이지 (비한국어 전용) =============
// 해외(th/vi/en) 방문자는 쓰는 메신저가 갈려서(태국 LINE·베트남 카톡·화교 WhatsApp) 단일 채널 강제 =
// 그 앱 안 쓰는 사람은 이탈 → 헤더 pill·하단 바가 /{lang}/consult.html 로 보낸다(채널 3개 + 원격 안내).
// ko 는 카카오 단일 채널이라 consult_channels 가 없고(빈 배열) → 기존 직행 링크 유지.
const __CONSULT = (window.__I18N__ && window.__I18N__.consultChannels) || [];
const __HAS_CONSULT_PAGE = __CONSULT.length > 1;
// 5칸(4탭 + 예약 or 상담)은 460px 플로팅 pill 에 넣으면 칸당 ~70px 라 라벨이 눌린다 →
// ko 처럼 엣지투엣지 플랫 바로. 지금은 ko(예약)·th/vi/en(상담) 둘 다 5칸이라 사실상 전 페이지.
const __IS_FULL_NAV = __IS_KO || __HAS_CONSULT_PAGE;
// 상담 페이지 경로 — 채널 목록·원격 안내는 이 페이지가 담당(옛 팝업 시트 대체).
const __CONSULT_HREF = `${__NAV_BASE}/consult.html`;
...
const __PATH_LANG_RE = /^\/(zh-hant|zh-hans|ko|th|vi|en)(\/.*)?$/;
...
${__IS_BLOG ? '' : `
  <nav class="t-bottom-nav${__IS_FULL_NAV ? ' t-bottom-nav--full' : ''}" aria-label="${tEsc('aria.menu', '메인 메뉴')}">
    ...
    </a>${__HAS_CONSULT_PAGE ? `
    <a href="${__CONSULT_HREF}" data-nav="consult">
