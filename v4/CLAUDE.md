...
| `consulting_qa` | id(=1 singleton), categories(jsonb), updated_at | 상담 매뉴얼 Q&A. `/consulting.html` 편집기가 읽고 씀 (migration 029). **v3 = `{version:3, cats:[…], markets:{…}}`** — 매뉴얼은 **하나**, 텍스트는 언어 맵 `{ko,en,th,vi,zh}`(ko=원본). `markets` 는 옛 시장별(th/kr/en/vn) 원본 보관 + **하위호환**(옛 페이지가 이 키가 없으면 "DB 비어있음"으로 보고 자기 v2 상태를 업로드해 v3 를 날린다). 레거시 배열·v2 는 로드 시 자동 마이그레이션 |
...
- 설문 어드민 메모: `069_intake_admin_note.sql` — `intake_submissions.admin_note text`. `/admin/intake` 접수함 = 리스트 각 행에 메모 미리보기(📝 앰버, 없으면 "메모 추가") → 클릭 시 편집 팝업(`AdminIntakePage.MemoModal`, `updateSubmissionNote`), 저장 시 리스트 즉시 갱신. 우측 상세엔 설문 내용만(메모 편집칸은 팝업으로 이관). ⚠️ **수동 적용 필요**(미적용 시 읽기는 graceful·저장은 42703 안내). 어드민 전용(anon 불필요). **(2026-07-21) 반려→대기 되돌리기**: 상세 푸터가 상태별 분기 — 반려된 제출은 반려 사유 배너(있을 때)+`[승인]`/`[🔄 다시 대기로]`(`reopenSubmission` = status→pending·reject_reason·reviewed_at 초기화, `onReopened` 은 선택 유지 리로드), 그 외는 `[승인]`/`[반려]`. **(2026-08-02) 「설문 수정」에 첨부 업로드**: 환자가 접수 뒤 이메일·채팅으로 보내온 X-ray·피검사를 직원이 붙인다(`IntakeSubmissionEditForm` 맨 아래 「첨부 파일」 섹션 + `adminUploadFiles`). ★**업로드는 [저장] 때 실행**(고르는 즉시 X — 취소 시 스토리지에 쓰레기가 남는다) · ★**파일 먼저, 설문 나중**(업로드 실패면 설문도 저장 안 함 — 저장은 됐는데 첨부만 날아간 상태가 제일 헷갈린다) · ★**슬롯 키는 타임스탬프**(`a{ts}-{i}`) — 공개 설문의 `0,1,2…` 를 이어 쓰면 겹칠 수 있고 스토리지가 `upsert:false` 라 **겹치는 순간 업로드 전체가 실패**한다. 검증은 공개 설문과 **같은 `validateFile`**(규칙이 갈리면 환자는 되는데 직원은 안 되는 일이 생긴다). 권한은 `018` 정책이 `bucket_id` 만 보고 경로를 안 봐서 anon 키 그대로 동작
...
| `/intake` · `/intake/:lang` | 환자 셀프 설문(공개 **7스텝** 마법사). **(2026-07-19 구조)** 위저드 본체 = 공유 컴포넌트 `components/IntakeWizard.tsx`(lang prop). **`/intake` = `GlobalIntakePage`** — 진입 시 **언어 선택 팝업**(en 맨 위·7언어) → 선택 언어로 위저드, 우상단 `🌐 언어 변경`. **`/intake/:lang` = `PublicIntakePage`**(어드민 공유용 직링크, 위저드에 위임) — 없는/잘못된 코드는 **en 폴백(default=영어)**. 언어 = ko/th/vi/en/zh-hans/zh-hant/ja/es/**ar**. **(2026-07-25 아랍어 추가)** — `IntakeLang`+`INTAKE_LABELS.ar`(89키 전부, MSA 격식)+`LANG_DEFAULT_COUNTRY.ar='SA'`+`GlobalIntakePage.LANGS`+`AdminIntakePage.SHARE_LANGS`+ar.yml 상담 링크. **RTL** = `types.INTAKE_RTL_LANGS` 단일 소스 → 위저드가 자기 래퍼에 `dir` 을 건다(옛 `dir="ltr"` 하드코딩 대체). ★`dir` 이 html 이 아니라 div 에 붙어 `_shell.css` 의 `html[dir=rtl]` 아랍어 폰트 규칙이 안 걸린다 → `src/index.css` 에 `[dir='rtl']` 스택 별도 추가. 방향 의존 Tailwind 는 논리 속성으로 교체(`ml-`→`ms-`·`text-left`→`text-start`·`pr-`→`pe-`). 국가명은 `Intl.DisplayNames` 라 아랍어 자동. `lang='ar'` insert 는 **DB 실측으로 확인**(CHECK 없음). ⚠️아랍어 원어민 감수 대기. (2026-07-19 간체·번체·일본어 추가 — `IntakeLang`+`INTAKE_LABELS`+`LANG_DEFAULT_COUNTRY`(CN/TW/JP)+`AdminIntakePage.SHARE_LANGS`, 전 문항 번역. `lang` 컬럼 CHECK 없어 마이그레이션 X. ⚠️중/일 원어민 감수 대기). 어드민 검토는 `/admin/intake`. **(2026-07-17 설문 통일)** 구글 폼 초진 설문지·`/diagnosis` 와 문항 세트 동일화 — 출생 정보(임신 주수·출생 몸무게·특이사항, StepBasic)·최근 1년 성장(StepGrowth)·**생활 습관 스텝 신설**(StepLifestyle — 취침/기상·운동 빈도·우유·식사 규칙성)·사춘기 상세 성별 분기(StepMedical — 남: 변성기·수염/체모, 여: 초경·유방 발달)·복용 약/영양제·추가 메모(StepCauses). 전부 `intake_survey` JSONB optional 필드(마이그레이션 X, 구접수 하위호환). 유입 경로에 `ai`(ChatGPT 등 AI) 옵션 4언어 추가. **(2026-08-02) 검색어 자유입력 신설** — `acquisition_keyword`(9언어), 유입 경로가 **google/naver/ai 일 때만** 노출(전원에게 물으면 노이즈). ★**구글이 검색어의 90%+ 를 가리므로**([[search_console_queries]]) 환자가 적어준 이 문장이 실제 검색어의 유일한 원본이다. 어드민 상세엔 「검색어」 보라 강조로 표시하고, **편집 폼에도 칸을 둬 상담 중 들은 검색어를 직원이 채워 넣을 수 있게** 했다(설문에 안 적는 사람이 대부분). `FieldShell` 에 `hint` 지원 추가(모든 필드 공용). 셀렉트는 코드값 저장 + `intakeLabels.optLabelKo` 로 어드민 한국어 표시. **(2026-07-25 국가 2분할)** 국적 한 칸이 **광고 지역 데이터**와 **예측키 성장기준** 두 용도로 쓰이고 있었다(태국인이 미국에 살면 어느 쪽?). → **거주 국가(필수)** + **성장 기준 국가**(거주국에서 프리필) 두 칸으로 분리, 둘 다 `SearchSelectField`(검색 가능 — 199개국엔 검색이 필수). ★**국가명은 번역 테이블이 아니라 `Intl.DisplayNames`** 로 얻는다(9언어 자동·번들 0). `shared/data/countries.ts` = `COUNTRY_CODES`(199) + `flagOf`(ISO→regional indicator) + `getCountries(lang)` + **`growthStandardOf(code)`**(→KR/TH/CN/US/ID/WHO). ★**부모에게 'CDC'·'WHO' 같은 내부 용어를 절대 안 보인다** — 나라만 고르게 하고 표준 매핑은 내부에서. 저장 = `intake_survey.growth_ref_country`·`growth_standard`(JSONB 라 마이그레이션 X). 🚨**(2026-08-02) 이 저장이 실제로는 빠져 있었다** — 화면·매핑 함수·9언어 번역까지 다 있는데 `publicIntakeService` 의 payload 에 없어서 **부모가 고른 성장 기준 국가가 조용히 버려지고 있었다**(문서만 앞서 있던 상태). 배선 + 어드민 상세 「성장 기준」 행 + 회귀 테스트 5케이스(`scripts/test/growth-standard-country.test.mjs`, 뮤테이션 검증). ★**`growthStandardOf` 는 `countries.ts` 에 이미 있다** — `growthStandard.ts` 에 새로 만들지 말 것(중복 구현할 뻔했다). 어드민용 레거시 API(`COUNTRIES`/`countryLabel`/`countryFlag`)는 한국어 고정으로 그대로 유지. **(2026-07-26 접수 텔레그램 알림)** 제출 성공 직후 `publicIntakeService.notifySubmission(token)` 이 ai-server `POST /api/intake/notify` 를 **fire-and-forget + `keepalive:true`**(완료 화면 전환으로 언마운트돼도 요청 생존)로 친다. ★**토큰만** 보내고 본문은 서버가 service_role 로 그 행을 다시 읽어 만든다(공개 엔드포인트라 요청자 입력을 그대로 쓰면 알림 위조 가능). insert 는 클라 그대로 — 스토리지 업로드가 섞여 있어 서버로 옮기면 전 흐름 재검증. 상세 [ai-server/CLAUDE.md](../ai-server/CLAUDE.md) §셀프 설문 접수 알림. **(2026-07-22 제출 오류 수정)** 생년월일이 년/월/일 자유 숫자 입력이라 **불가능한 날짜**(2월31일·13월·30일달의 31일·오타)를 넣으면 DB insert 가 Postgres `22008 date/time field value out of range` 로 죽고 화면엔 이유 없는 ⚠ 만 떴다(필수라 입력은 했지만 값이 불가능 → "선택 몇 개 안 넣어서"로 오인). ★DB 실삽입 테스트로 확진: 7언어 전부·null 선택필드 전부 정상, **불가능 날짜만 실패**. 수정 = `IntakeWizard.isRealDate`(Date 왕복 검증, 윤년 2/29 정확) 로 StepBasic "다음" 에서 걸러 **생년월일 필드에 즉시 오류**(`invalidBirth` 7언어) + 제출 실패 메시지도 밋밋한 ⚠ → `submitError`(7언어) 문장으로 |
| `/banner-admin` | AdminWebsitePage (PIN 보호) |
| `/consulting.html` | 상담 매뉴얼 Q&A 편집기 (정적 HTML, noindex). **매뉴얼 1개 + 언어 토글(한글·English·ไทย·Tiếng Việt·中文)** — 한글로 작성하고 🌐 버튼으로 언어별 번역본을 채운다(ai-server `/api/marketing/translate-text` **`style:'formal'`**, dev-only). 카테고리/질문/답변 + 질문별 공개토글, Supabase `consulting_qa` 싱글톤에 저장, supabase-js CDN 직접 연동. admin 사이드바 "상담 매뉴얼"(`/admin/consulting`)이 iframe 으로 임베드. **(2026-07-22) 시장 4탭 → 일반 매뉴얼 1개로 통합**(6카테고리 29문항, 첫 상담 경로 2가지[현지 검사→원격 상담→한국 치료 / 한국 검사→상담→치료]). 🚨편집마다 싱글톤 행 전체를 upsert → 열린 탭이 남의 저장을 덮어씀 → `updated_at` 비교 가드(`lastSynced` 는 **upsert 응답에서 되받을 것**, ISO `Z` vs timestamptz `+00:00` 오탐). ★AI 서버 포트는 **4000**(옛 코드의 3001 은 오기 = 번역이 한 번도 동작한 적 없었음). 상세 [[consulting_manual]] |
