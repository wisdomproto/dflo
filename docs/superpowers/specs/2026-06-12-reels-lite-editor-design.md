...
날짜: 2026-06-12 · 상태: ✅ 구현 완료(2026-06-14, P1~P4 + E2E). 설계는 리뷰 반영 v2 (3-렌즈 멀티에이전트 리뷰 blocking 3·advisory 20 반영)
...
- **잡 처리 순서** (단계마다 status + updated_at 갱신, 장단계는 60s heartbeat):
  ...
  4. **upload_preview**(full만): 립싱크 mp4 + 청크 wav들 → R2
     folder `marketing/reels/preview/{articleId}/{lang}` (ai-server 업로드 API가
     folder만 받고 키를 서버 생성하며 비ASCII를 strip — 한글 slug 금지, 최종
     URL은 서버 반환값이 정본) → `reel_runtime`의
