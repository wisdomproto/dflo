...
- 🚨 **"Meta 연결이 없습니다" 는 원인 셋을 뭉갠 메시지였다** (2026-07-26 수정): `getBundle()` 이 ①조회 실패 ②저장된 연결 없음 ③복호화 실패 를 전부 `null` 로 반환 — ★**`getEncKey()` 가 `try` 안**이라 `META_TOKEN_ENC_KEY` 누락도 '연결 없음' 으로 둔갑한다(로컬에서 발행이 막히던 것도 실은 이것). `getBundleResult()` 로 원인을 갈라 큐 행 `error_message` 에 남긴다(`getBundle()` 은 시그니처 유지 + 로그). 같은 연결로 **FB 는 되고 IG 는 안 되던 간헐 실패**는 replica 차이가 아니라(**replica 1개**, EU West) **조회 자체가 튀는 것** → `getBundleResult()` 가 조회를 **3회 재시도**(400·800ms). 읽기 한 번 실패로 발행이 15/60/180분 밀리고 영어 IG 는 재시도 3번을 다 태우고 최종 failed 한 적이 있다(7/23·7/24).
...
- **자동 재시도** (migration 064 `retry_count`): `publishExecutor.fail()` 이 발행 실패 시 **미발행 + retry_count<3 이면 `scheduled` 로 백오프 재예약**(15분 → 1시간 → 3시간), 소진 시 최종 `failed`. 스케줄러가 `scheduled` 만 집으므로 **기존 `failed` 행은 영향 없음**(미래 실패만 재시도). **중복 방지**: 이미 발행됨(`published_url`/`platform_post_id`) 이면 재시도 금지 + `fetchPermalink` 실패를 비치명적으로(post 생성됐는데 permalink만 실패 → 재시도→중복 방지). `retry_count` 컬럼 미적용이면 graceful 하드실패(현행). board 엔 `[자동 재시도 N/3 · X분 후] …` 로 표시.
