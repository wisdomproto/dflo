// 예측 성인키(PAH) 계산 — Node 용 LMS 모듈.
// v4/src/features/bone-age/lib/growthStandard.ts 의 KR LMS + 계산 로직.
// hospital_measurements.pah 백필 시에만 사용. 일반 스토리 dump 는 이 결과를
// 거치지 않고 DB 의 pah 컬럼을 직접 읽는다.
...
export function predictAdultHeightByBonePercentile(currentHeight, boneAge, gender) {
  ...
  const cur = interpolateLMS(boneAge, table);
  const adult = interpolateLMS(18, table);
  if (!cur || !adult) return null;
  const z = zScoreFromLMS(currentHeight, cur);
  return heightFromLMS(adult, z);
}
