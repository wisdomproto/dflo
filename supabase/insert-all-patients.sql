...
BEGIN
    ...
    -- 필수 정보 추가
    INSERT INTO child_required_info (child_id, father_height, mother_height, target_height, notes)
    VALUES (child_id_1, 170, 156, 180, '-알레르기 비염 있음\n-1년전 타병원 PAH 176cm');
    
    -- 측정 기록 추가
    INSERT INTO measurements (child_id, measured_date, height, weight, actual_age, bone_age, pah, notes)
    VALUES 
        (child_id_1, '2023-04-25', 155.7, 58.6, 12.25, 13.17, 172, ''),
...
BEGIN
    ...
    INSERT INTO child_required_info (child_id, father_height, mother_height, target_height, notes)
    VALUES (child_id_5, 176, 162, 163, '-2년전 타병원서 뼈나이 +8개월 진단\n-수면이 좋지 못한 편');
    
    INSERT INTO measurements (child_id, measured_date, height, weight, actual_age, bone_age, pah, notes)
    VALUES 
        (child_id_5, '2024-01-30', 148, 36.4, 11.0, 12.25, 157, '-지난 주 초경시작\n-루프린 병행 바로 시작'),
        ...
        (child_id_5, '2025-08-05', 155.8, 39, NULL, NULL, NULL, '-해외 체류 중 감기몸살로 주사 못맞음\n-생리 아직 없어요');
END $$;

-- ================================================
-- 나머지 40명은 동일한 방식으로 추가...
-- (데이터가 매우 길어서 처음 5명만 작성)
-- ================================================

-- 완료 메시지
SELECT '✅ 5명의 환자 데이터 입력 완료! (나머지 40명 추가 필요)' AS result;
