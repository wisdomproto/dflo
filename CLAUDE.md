...
- **유튜브 쇼츠 다국어 재현**(@187growup 채널 78개 → th/en/vi/zh 번역): 토킹헤드 영상을 클론음성(ElevenLabs)+LatentSync 립싱크로 현지화. **factory**: `src/shorts/<한글slug>/script.json`(대본+자막+위치 `capY`/`cover`) + `_shared/FaithfulShort.tsx`(제목·자막·로고 범용 컴포넌트, perchunk/linear/footage 3모드) + `scripts/gen-tts-short.mjs <slug> <lang>`(범용 TTS). 파이프라인: yt-dlp 다운→ko자막→번역→TTS→footage렌더→LatentSync(no-face패치)→최종합성. 분석: `scripts/{analyze-shorts,merge-analysis}.py`(얼굴검출+제목분류→토킹헤드/보이스오버/몽타주). **초경편 5개 언어 완성**(ko/th/en/vi/zh, `out/shorts/초경/초경-{lang}.mp4`). **핵심 교훈: 자막 마스킹은 `cover`(전체폭 고정밴드)로 — 언어마다 footage 타이밍이 달라 `capY` 점 위치는 어긋나지만 cover는 언어무관**. index.tsx가 lang별 컴포지션(헬퍼 `mkPreview/Footage/Final`). ⚠️ Remotion 컴포지션 ID 한글 불가→로마자(`chogyeong`), 폴더/파일만 한글. **zh는 ElevenLabs 광동어 미지원→만다린 발음**(자막은 번체중국어). 상세·플레이북·다음작업은 memory `youtube_shorts_recreation.md`
