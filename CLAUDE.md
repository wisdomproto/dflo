...
- **Compositions**:
  ...
  - `HeightReelsKRMarketing` (마케팅 풀 퍼널, 1015프레임 ~34초, KR) — 8씬: 키성장·골든타임 인트로(`FearIntroScene` 🆕) → 원장·실적·병원 통합(`ClinicScene marketing`: 학회+포지셔닝+95%) → VS 차별화(`VsScene` 🆕: 단순주사 vs 6가지 통합관리) → 외국 셀럽·아역배우 포지셔닝(`CelebScene` 🆕) → 사례(`CasesScene line`) → 예측키 측정 데모(`HookScene hideCta`+`InputScene`) → 결과+홈페이지 무료측정(`ResultScene footerNote`) → 메신저 CTA(`CtaPromoScene heading`). 공유 씬은 optional prop 분기로 **회귀 0** (`statsCount2 90→95`만 의도된 공유 상향). 카피 원칙·자산 placeholder는 `memory/marketing_reel_kr.md` 참조 (시청자 미측정 단정 ❌·협박 ❌→긍정 골든타임·"또래보다 작은" 타겟협소 ❌→"우리 아이 키 성장" 보편)
- **홍보 릴 핵심 설계**:
