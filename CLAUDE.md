...
- **유튜브 쇼츠 다국어 재현**(@187growup 채널 78개 → th/en/vi/zh 번역): 토킹헤드 영상을 클론음성(ElevenLabs)+LatentSync 립싱크로 현지화. **factory**: `src/shorts/<한글slug>/script.json`(대본+자막+위치 `capY`/`cover`) + `_shared/FaithfulShort.tsx`(제목·자막·로고 범용 컴포넌트, perchunk/linear/footage 3모드) + `scripts/gen-tts-short.mjs <slug> <lang>`(범용 TTS). 파이프라인: yt-dlp 다운→ko자막→번역→TTS→footage렌더→LatentSync(no-face패치)→최종합성. 분석: `scripts/{analyze-shorts,merge-analysis}.py`(얼굴검출+제목분류→토킹헤드/보이스오버/몽타주). **완성: 초경(ko/th/en/vi/zh)·부작용(ko/en/th)·몽정(ko/en/th)** — `out/shorts/<slug>/<slug>-{lang}.mp4`. **2개 영상 카테고리**: 🎙️토킹헤드(초경·부작용)=클론음성+**립싱크** / 📋몽타주(몽정, 원장 얼굴 없이 일러스트·스톡)=**립싱크 X**(스톡 속 얼굴 보호 — videoMode=perchunk가 곧 최종). **핵심: 자막 마스킹은 `cover`(전체폭 고정밴드) — 언어마다 footage 타이밍 달라 `capY` 점은 어긋나지만 cover는 언어무관**. 헤더 박스색 = `headerStyle`(초경 노랑/부작용 빨강/몽정 초록). 번역은 **간결·punchy**(현지어가 짧아 영상도 짧음 — 숏폼엔 유리). index.tsx가 lang별 컴포지션(헬퍼 `mkPreview/Footage/Final`). ⚠️ Remotion ID 한글 불가→로마자, 폴더/파일만 한글. **zh=ElevenLabs 광동어 미지원→만다린**(자막 번체). 상세·플레이북·다음작업은 memory `youtube_shorts_recreation.md`
